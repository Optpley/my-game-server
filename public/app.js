...
function initWebSocket() {
  ...
  ws.onmessage = (event) => {
    ...
    if (data.type === "global_stats") {
      // update mode counts badges
      if (data.modeCounts) {
        Object.keys(data.modeCounts).forEach((m) => {
          const el = document.querySelector(`.mode-count[data-mode="${m}"]`);
          if (el) el.textContent = data.modeCounts[m] || 0;
        });
      }
    }
  };
...
function initTabs() {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      Object.keys(tabs).forEach((k) => tabs[k].classList.toggle("active-tab", k === tab));
    });
  });
}
...
function initAdmin() {
  ...
  adminBtn.addEventListener("click", () => {
    if (!currentUser || currentUser.username !== "Capibaraboyonrealnokrutoy") {
      tg.showPopup({ title: "Недоступно", message: "У вас нет доступа к админ‑панели", buttons: [{ id: "ok", type: "default", text: "Ок" }] });
      return;
    }
  ...
  adminGiveBtn.addEventListener("click", async () => {
    ...
    const res = await fetch("/api/admin/give-stars", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ adminSecret: "dev_secret", username, amount }),
    });
    const data = await res.json();
    tg.showAlert(data.ok ? "Звёзды выданы" : "Ошибка: " + (data.error || "unknown"));
  });
  ...
  adminBroadcastBtn.addEventListener("click", async () => {
    ...
    const res = await fetch("/api/admin/broadcast", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ adminSecret: "dev_secret", text }),
    });
    const data = await res.json();
    tg.showAlert(data.ok ? `Отправлено: ${data.sent}` : "Ошибка: " + (data.error || "unknown"));
  });
  ...
  tournamentCreateBtn.addEventListener("click", async () => {
    ...
    const res = await fetch("/api/admin/tournament", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ adminSecret: "dev_secret", modes, bet, prize }),
    });
    const data = await res.json();
